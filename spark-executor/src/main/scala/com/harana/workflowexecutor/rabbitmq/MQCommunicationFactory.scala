...
import com.newmotion.akka.rabbitmq.{Channel, ChannelActor, ChannelMessage, RichConnectionActor}
...
case class MQCommunicationFactory(system: ActorSystem, connection: ActorRef, mqMessageSerializer: MQSerializer, mqMessageDeserializer: MQDeserializer) extends Logging {

  type QueueName = String
  val exchangeType = "topic"

  /** @return future containing queue name and channel actor so we can later unsubscribe and close the channel
    */
  def registerSubscriber(topic: String, subscriber: ActorRef): Future[ChannelSetupResult[QueueName]] = {
    ...
    val actorProps = NotifyingChannelActor.props[QueueName](channelConnected, setupSubscriber(topic, subscriber))
  ...
  private def setupSubscriber(topic: String, subscriber: ActorRef)(channel: Channel, self: ActorRef): QueueName = {
    val queueName = MQCommunication.queueName(topic)
    val queue = channel.queueDeclare(queueName, false, false, true, new util.HashMap[String, AnyRef]()).getQueue
    channel.queueBind(queue, MQCommunication.Exchange.harana, topic)
  ...
  private def createMQPublisher(topic: String) = withTimeoutClue(channelTimeoutMsg) {
    ...
    val channelActor = connection.createChannel(ChannelActor.props(), Some(publisherName))(timeout = 10.seconds)
    MQPublisher(MQCommunication.Exchange.harana, mqMessageSerializer, channelActor)
  }
  ...
  private def createMQBroadcaster(exchange: String): MQPublisher = {
    ...
    def setupChannel(channel: Channel, self: ActorRef) = channel.exchangeDeclare(exchange, "fanout")

    val channelActor: ActorRef = connection.createChannel(ChannelActor.props(setupChannel), Some(publisherName))(timeout = 10.seconds)
...
object MQCommunicationFactory {
  ...
  /** NotifyingChannelActor extends ChannelActor behaviour by completing a promise after connecting to a channel. In the
    * newmotion's library for RMQ, it's impossible to synchronously connect to a channel. NotifyingChannelActor
    * resolves this issue by allowing to wait for the promise to complete.
    *
    * <br><br> More on the issue:<br> As the documentation says, to synchronously create a channel one have to call
    ...
    */
  class NotifyingChannelActor[T <: AnyRef](channelConnected: Promise[ChannelSetupResult[T]], setupChannel: (Channel, ActorRef) => T) extends {
      val setupPromise = Promise[ChannelSetupResult[T]]()
      val setupResults: Future[ChannelSetupResult[T]] = setupPromise.future
    }
      with ChannelActor({ case (channel, channelActor) =>
  ...
  object NotifyingChannelActor {
    def props[T <: AnyRef](channelConnected: Promise[ChannelSetupResult[T]], setupChannel: (Channel, ActorRef) => T) =
      Props(new NotifyingChannelActor(channelConnected, setupChannel))
  }
}