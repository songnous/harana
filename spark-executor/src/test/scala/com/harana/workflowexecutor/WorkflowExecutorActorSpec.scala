...
import com.harana.sdk.shared.models.designer.flow.graph.node.NodeStatus.{Aborted, Completed, Draft, Queued, Running}
...
import com.harana.workflowexecutor.WorkflowManagerClientActorProtocol.{GetWorkflow, SaveState, SaveWorkflow}
...
class WorkflowExecutorActorSpec
  ...
  "SessionWorkflowExecutorActor" when {
    ...
    "received Init" should {
      ...
      "send workflow with results" in {
        ...
        val (probe, wea, _, statusListeners, _, _) = fixture(workflow, useWorkflowManagerProbe = false)
    ...
    "received Launch" should {
      ...
      "send status update" in {
        ...
        val (probe, wea, _, statusListeners, _, testWMClientProbe) = initializedStateFixture(workflow)
      ...
      "infer graph knowledge, fail graph and send status on exception" in {
        ...
        verifyStatus(statusListeners)(executionStatus => executionStatus.error.isDefined shouldBe true)
        eventually {
          val saveState = testWMClientProbe.expectMsgClass(classOf[SaveState])
          saveState.workflowId shouldBe workflow.id
      ...
      "after relaunch handle only changed nodes" in {
        val workflow = workflowInvalidInference(Workflow.Id.randomId)
        val (_, wea, _, statusListeners, _, testWMClientProbe) = finishedStateFixture(workflow)
        ...
        eventually {
          statusListeners.foreach { receiver =>
            ...
            executionStatus.nodesStatuses(node2.id) shouldBe a[Running]
          }
          wea.underlyingActor.execution.graph.states(node1.id).nodeState.nodeStatus shouldBe a[Completed]
        }
        eventually {
          val saveState = testWMClientProbe.expectMsgClass(classOf[SaveState])
          saveState.workflowId shouldBe workflow.id
          saveState.state.nodesStatuses.size shouldBe 1
          saveState.state.nodesStatuses(node2.id) shouldBe a[Running]
        }
      }
    }
    ...
    "received NodeCompleted" should {
      ...
      "tell graph about it" in {
        ...
        val workflow = workflowWithResults(Workflow.Id.randomId)
        val states: Map[Id, NodeStateWithResults] = Map(node1.id -> nodeState(Draft()), node2.id -> nodeState(Draft()))
        when(statefulWorkflow.changesExecutionReport(any())).thenReturn(ExecutionReport(states.mapValues(_.nodeState)))
        when(statefulWorkflow.currentExecution).thenReturn(Execution(StatefulGraph(workflow.graph, states, None)))
        val (probe, wea, _, statusListeners, _, testWMClientProbe) = launchedStateFixture(workflow)
        ...
        eventually {
          statusListeners.foreach(receiver => receiver.expectMsgClass(classOf[ExecutionReport]))
          verify(statefulWorkflow).nodeFinished(
            completedId,
            nodeCompletedMessage.results.entitiesId,
            nodeCompletedMessage.results.reports,
            nodeCompletedMessage.results.actionObjects
          )
        }
        eventually {
          val saveState = testWMClientProbe.expectMsgClass(classOf[SaveState])
      ...
      "launch ready nodes" when {
        "ready nodes exist" in {
          val statefulWorkflow = mock[StatefulWorkflow]
          val workflow = workflowWithResults(Workflow.Id.randomId)
          val states = Map(node1.id -> nodeState(Draft()), node2.id -> nodeState(Draft()))
          when(statefulWorkflow.changesExecutionReport(any())).thenReturn(ExecutionReport(states.mapValues(_.nodeState)))
          when(statefulWorkflow.currentExecution).thenReturn(
            RunningExecution(
              ...
              StatefulGraph(
                ...
                Map(
                  node1.id -> workflows.NodeStateWithResults(
                    NodeState(Running(Instant.now), Some(EntitiesMap())),
                    Map(),
                    None
                  )
                ),
          ...
          val (probe, wea, _, _, _, testWMClientProbe) = launchedStateFixture(workflow)
          wea.underlyingActor.statefulWorkflow = statefulWorkflow
    ...
    "receive StructUpdate" should {
      "update state, send update to WM and to editor" in {
        ...
        when(nodeExecutorFactory.createGraphNodeExecutor(any(), any(), any(), any())).thenReturn(wnea.ref)

        val wea = TestActorRef(
        ...
        val workflowWithResults = mock[WorkflowWithResults]
  ...
  private def sendLaunch(probe: TestProbe, wea: TestActorRef[WorkflowExecutorActor], nodes: Set[Node.Id]): Unit =
    probe.send(wea, Launch(nodes))

  private def verifyStatus(receivers: Seq[TestProbe])(f: (ExecutionReport) => Unit): Unit = {
    eventually {
      receivers.foreach(receiver => f(receiver.expectMsgClass(classOf[ExecutionReport])))
    }
  }

  private def becomeLaunched(wea: TestActorRef[WorkflowExecutorActor], workflow: WorkflowWithResults): Unit = {
    val statefulWorkflow = StatefulWorkflow(wea.underlyingActor.executionContext, workflow, Execution.defaultExecutionFactory)
    statefulWorkflow.launch(workflow.graph.nodes.map(_.id))
    wea.underlyingActor.statefulWorkflow = statefulWorkflow
    wea.underlyingActor.context.become(wea.underlyingActor.launched())
  }

  private def becomeFinished(wea: TestActorRef[WorkflowExecutorActor], workflow: WorkflowWithResults): Unit = {
    val statefulWorkflow = new StatefulWorkflow(
      ...
      Execution(StatefulGraph(workflow.graph, workflow.graph.nodes.map(_.id -> completedState()).toMap, None)),
  ...
  private def mockResults(size: Int) = {
    val ids = (1 to size).map(_ => Entity.Id.randomId)
    val reports = ids.map(id => id -> mock[ReportContent]).toMap
    val actionObjects = ids.map(id => id -> mock[ActionObjectInfo]).toMap
    NodeExecutionResults(ids, reports, actionObjects)
  }

  private def nodeFailed() = {
  ...
  private def fixture(workflow: WorkflowWithResults, useWorkflowManagerProbe: Boolean = true) = {
    ...
    val wmClient =
      if (useWorkflowManagerProbe)
        ...
        wmClientActor(Map[Id, WorkflowWithResults](workflow.id -> workflow))
  ...
  private def completedState(): NodeStateWithResults = {
    ...
    val now = Instant.now
    val actionObjects: Map[Id, DataFrame] = Map(entityId -> new DataFrame())
    workflows.NodeStateWithResults(
      NodeState(
        Completed(now.minusHours(1), now, Seq(entityId)),
        Some(EntitiesMap(actionObjects, Map(entityId -> ReportContentTestFactory.someReport)))
      ),
      actionObjects,
      None
    )
  ...
  private def wmClientActor(workflows: Map[Id, WorkflowWithResults]): ActorRef =
  ...
  val node1 = Node(Node.Id.randomId, ReadDataFrame().setStorageType(inputFile))
  val node2 = Node(Node.Id.randomId, new WriteDataFrame().setStorageType(outputFile))
  val invalidNode = Node(Node.Id.randomId, new WriteDataFrame())

  private def someFilePath = FilePath(FileScheme.File, "/tmp/doesnt_matter").fullPath

  private def workflowWithResults(id: Id) = WorkflowWithResults(
    id,
    FlowType.Batch,
    "1.0.0",
  ...
  private def workflowInvalidInference(workflowId: Id) =
    workflowWithResults(workflowId).copy(
      ...
      executionReport = ExecutionReport(
        Map(node1.id -> NodeState.draft, node2.id -> NodeState.draft, invalidNode.id -> NodeState.draft)
      )
    )

  private class TestWMClientActor(workflows: Map[Id, WorkflowWithResults]) extends Actor with Logging {
    override def receive: Receive = { case GetWorkflow(id) =>
