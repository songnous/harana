...
public class Decryption {
    ...
    public Decryption(SecretKeySpec key, BlobStore blobStore, BlobMetadata meta, long offset, long length) throws IOException {
        ...
        // detect multipart
        if (lastPartPadding.getPart() > 1 && meta.getSize() > (lastPartPadding.getSize() + Constants.PADDING_BLOCK_SIZE)) {
            unencryptedSize = lastPartPadding.getSize();
            encryptedSize = lastPartPadding.getSize() + Constants.PADDING_BLOCK_SIZE;
            ...
            while (encryptedSize < meta.getSize()) {
                ...
                long startAt = (meta.getSize() - encryptedSize) - Constants.PADDING_BLOCK_SIZE;
    ...
    public final long calculateEndAt(long endAt) {
        ...
        if (partList.size() > 1) {
            ...
            for (Map.Entry<Integer, PartPadding> part : partList.descendingMap().entrySet()) {
                fefffff
