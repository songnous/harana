...
trait GridSearchInfo
  ...
  val description = "Uses Cross-validation to find the best set of parameters for input estimator. User can specify a list of parameter values to test and compare."
  val since = Version(1, 0, 0)
  val category = HyperOptimization

  val estimatorParameters = new GridSearchParameter("Parameters of input Estimator", Some("These parameters are rendered dynamically, depending on type of Estimator."), inputPort = 0)
  ...
  val evaluatorParameters = new DynamicParameter("Parameters of input Evaluator", Some("These parameters are rendered dynamically, depending on type of Evaluator."), inputPort = 2)
  ...
  val numberOfFoldsParameter = NumericParameter("number of folds", None,
