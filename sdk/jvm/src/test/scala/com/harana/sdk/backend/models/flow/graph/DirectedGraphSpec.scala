...
class DirectedGraphSpec extends AnyFunSuite with Matchers with Serialization with Logging with MockitoSugar with GraphTestSupport {
  ...
  test("Graph with two nodes should have size 2") {
    ...
    val node2 = randomNode(ActionTypeTypeA1ToA())
  ...
  test("Programmer can validate if graph doesn't contain a cycle") {
    ...
    val node1 = randomNode(ActionTypeTypeA1ToA())
    val node2 = randomNode(ActionTypeTypeA1A2ToA())
    val node3 = randomNode(ActionTypeTypeA1ToA())
    val node4 = randomNode(ActionTypeTypeA1ToA())
  ...
  test("Complicated Graph can be sorted topologically") {
    ...
    val node2 = randomNode(ActionTypeTypeA1ToA())
    val node3 = randomNode(ActionTypeTypeA1ToA())
    val node4 = randomNode(ActionTypeTypeA1A2ToA())
    val node5 = randomNode(ActionTypeTypeA1ToA())
    val node6 = randomNode(ActionTypeTypeA1ToA())
    val node7 = randomNode(ActionTypeTypeA1A2ToA())
  ...
  test("Graph's nodes have correct predecessors and successors") {
    ...
    val node1 = randomNode(ActionTypeTypeCreateA1())
    val node2 = randomNode(ActionTypeTypeA1ToA())
    val node3 = randomNode(ActionTypeTypeA1ToA())
    val node4 = randomNode(ActionTypeTypeA1A2ToA())
    val nodes = Set(node1, node2, node3, node4)
