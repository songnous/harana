...
object ToVectorConversions {
  implicit def dKnowledgeSingletonToVector[T1 <: ActionObjectInfo](t: (Knowledge[T1], InferenceWarnings)) = (Vector(t._1), t._2)
  implicit def dKnowledgeTuple2ToVector[T1 <: ActionObjectInfo, T2 <: ActionObjectInfo](t: ((Knowledge[T1], Knowledge[T2]), InferenceWarnings)) = {
    val (k, w) = t
    (Vector(k._1, k._2), w)
  }

  implicit def dKnowledgeTuple3ToVector[T1 <: ActionObjectInfo, T2 <: ActionObjectInfo, T3 <: ActionObjectInfo](t: ((Knowledge[T1], Knowledge[T2], Knowledge[T3]), InferenceWarnings)) = {
    val (k, w) = t
...
trait Action0To1[TO_0 <: ActionObjectInfo] extends Action {
  ...
  final def inferKnowledgeUntyped(knowledge: Vector[Knowledge[ActionObjectInfo]])(context: InferContext) =
...
trait Action0To2[TO_0 <: ActionObjectInfo, TO_1 <: ActionObjectInfo] extends Action {
  ...
  final def inferKnowledgeUntyped(knowledge: Vector[Knowledge[ActionObjectInfo]])(context: InferContext): (Vector[Knowledge[ActionObjectInfo]], InferenceWarnings) =
    inferKnowledge()(context)
...
trait Action1To0[TI_0 <: ActionObjectInfo] extends Action {
  ...
  final def inferKnowledgeUntyped(knowledge: Vector[Knowledge[ActionObjectInfo]])(context: InferContext) = {
    ...
    (Vector.empty[Knowledge[ActionObjectInfo]], InferenceWarnings.empty)
  }
