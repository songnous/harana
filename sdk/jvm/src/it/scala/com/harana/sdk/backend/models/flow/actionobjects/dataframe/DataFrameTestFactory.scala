...
trait DataFrameTestFactory {
  ...
  def testDataFrame(dataFrameBuilder: DataFrameBuilder, sparkContext: SparkContext) =
    dataFrameBuilder.buildDataFrame(testSchema, testRDD(sparkContext))

  def oneValueDataFrame(dataFrameBuilder: DataFrameBuilder, sparkContext: SparkContext) =
    dataFrameBuilder.buildDataFrame(testSchema, sameValueRDD(sparkContext))

  def allTypesDataFrame(dataFrameBuilder: DataFrameBuilder, sparkContext: SparkContext) =
    dataFrameBuilder.buildDataFrame(allTypesSchema, allTypesRDD(sparkContext))
  ...
  val allTypesSchema: StructType = StructType(
    Seq(
      ...
      StructField(vectorColumnName, new com.harana.spark.Linalg.VectorUDT)
    )
  )

  def testRDD(sparkContext: SparkContext): RDD[Row] = sparkContext.parallelize(
    Seq(
  ...
  def sameValueRDD(sparkContext: SparkContext): RDD[Row] = sparkContext.parallelize(
    Seq(
  ...
  def allTypesRDD(sparkContext: SparkContext): RDD[Row] = sparkContext.parallelize(
    Seq(
      Row(
        Byte.box(0),
        Short.box(0),
  ...
  private def timestampUTC(year: Int, month: Int, day: Int, hour: Int = 0, minutes: Int = 0, seconds: Int = 0) =
    new Timestamp(dateTimeFromUTC(year, month, day, hour, minutes, seconds).getMillis)
}
