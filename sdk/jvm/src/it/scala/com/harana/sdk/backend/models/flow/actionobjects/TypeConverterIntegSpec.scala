...
import com.harana.sdk.shared.models.flow.actionobjects.TargetTypeChoice
...
class TypeConverterIntegSpec extends IntegratedTestSupport with MultiColumnTransformerTestSupport with TransformerSerialization {

  import IntegratedTestSupport._
  import TransformerSerialization._

  var inputDataFrame: Option[DataFrame] = None
    ...
    "converting columns to String".which {
      ...
      "are Strings" should {
        "return the same values" in {
          ...
          assertDataFramesEqual(converted, inputDataFrame.get)
        }
      }
      "are Booleans" should {
  ...
  override def beforeAll() = {
    super.beforeAll()
    inputDataFrame = Some(createDataFrame(inputRows, schema))
  }
  ...
  val timestampColumn = {
    ...
    def timestampStringToSparkDouble(ts: String) = DateTimeUtils.stringToTimestamp(UTF8String.fromString(ts), ZoneId.systemDefault()).get.toDouble / 1000000.0d

    val asDouble = Seq(timestampStringToSparkDouble(dateTime1String), null, timestampStringToSparkDouble(dateTime2String))
    ColumnContainer(original, asDouble, asString, emptyMetadata)
  }
  ...
  val columns = Seq(doubleColumn, numStrColumn, strColumn, boolColumn, timestampColumn, longColumn, integerColumn, floatColumn, byteColumn,
    shortColumn, decimalColumn)
  ...
  def to(dataType: DataType, ids: Set[Int], newSchema: Option[StructType])(f: ColumnContainer => Seq[Any]) = {
    ...
    val updatedSchema = newSchema.getOrElse(ids.foldLeft(schema) {
      case (oldSchema, index) =>
        StructType(oldSchema.updated(index, oldSchema(index).copy(dataType = dataType, metadata = new MetadataBuilder().build())))
    })
    createDataFrame(rows, updatedSchema)
  }

  def toDouble(ids: Set[Int]) = to(DoubleType, ids, None)(_.asDouble)
  def toString(ids: Set[Int]) = to(StringType, ids, None)(_.asString)
  ...
  private def useTypeConverter(
      ...
      dataFrame: DataFrame = inputDataFrame.get) = {
    ...
    action.applyTransformationAndSerialization(tempDir, dataFrame)
  }

  def transformerName = "TypeConverter"

  def inputType: DataType = DoubleType
  def outputType: DataType = StringType

  def transformer = TypeConverter().setTargetType(StringTargetTypeChoice())
