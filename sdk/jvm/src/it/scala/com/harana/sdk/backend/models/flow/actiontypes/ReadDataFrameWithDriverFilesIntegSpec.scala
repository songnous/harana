...
import com.harana.sdk.backend.models.flow.actiontypes.exceptions.BacktickInColumnNameError
...
import com.harana.sdk.backend.models.flow.actiontypes.readwritedataframe.{FileScheme, UnknownFileSchemaForPath}
...
import com.harana.sdk.shared.models.flow.actiontypes.inout.{CsvParameters, InputFileFormatChoice, InputStorageTypeChoice}

class ReadDataFrameWithDriverFilesIntegSpec extends IntegratedTestSupport with BeforeAndAfter with TestFiles {
  ...
  "ReadDataFrame" should {
    ...
    "throw on csv with column names containing backticks" in {
      a[BacktickInColumnNameError] should be thrownBy readDataFrame(
      ...
      val rdf = ReadDataFrame()
        .setStorageType(
          new InputStorageTypeChoice.File()
            .setSourceFile(File.pathPrefix + "/some_path/some_file.parquet")
  ...
  def expectedDataFrame(rows: Seq[Row], schema: StructType) = createDataFrame(rows, schema)
  ...
  def schemaWithDefaultColumnNames(types: Seq[DataType]) =schemaOf(generatedColumnNames(types.length), types)
