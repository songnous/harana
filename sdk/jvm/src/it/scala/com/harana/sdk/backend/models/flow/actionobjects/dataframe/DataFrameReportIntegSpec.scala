...
import com.harana.sdk.shared.models.flow.report.{DiscreteDistribution, NoDistribution, Table}
...
class DataFrameReportIntegSpec extends IntegratedTestSupport with DataFrameTestFactory {
  ...
  "DataFrame" should {
    ...
    "generate report with data sample table" when {
      ...
      "DataFrame has missing values" in {
        ...
        val rdd = sparkContext.parallelize(List(Row(null, new Timestamp(now.getMillis)), Row(exampleString, null)))
        val schema = StructType(List(StructField(nameColumnName, StringType), StructField(birthDateColumnName, TimestampType)))
        val dataFrame = executionContext.dataFrameBuilder.buildDataFrame(schema, rdd)

        val report = dataFrame.report()

        val dataSampleTable = report.content.tableByName(DataFrameReportGenerator.DataSampleTableName).get
        dataSampleTable.columnNames shouldBe Some(List(nameColumnName, birthDateColumnName))
        dataSampleTable.rowNames shouldBe None
        dataSampleTable.values shouldBe List(List(None, Some(DateTimeConverter.toString(now))), List(Some(exampleString), None))
      }
    ...
    "generate report with correct column types" in {
      ...
      val dataSampleTable = dataFrame.report().content.tableByName(DataFrameReportGenerator.DataSampleTableName).get
    ...
    "generate simplified report with only schema table" when {
      ...
      "number of column in schema exceeds threshold" in {
        ...
        val expectedValues = dataFrame.schema.get.fields.zipWithIndex.map { case (field, index) =>
    ...
    "generate correct report" when {
      "DataFrame is empty" in {
        ...
        val emptyDataFrame = executionContext.dataFrameBuilder.buildDataFrame(schema, sparkContext.parallelize(Seq.empty[Row]))
        val report = emptyDataFrame.report()

        val dataSampleTable = report.content.tableByName(DataFrameReportGenerator.DataSampleTableName).get
        dataSampleTable.columnNames shouldBe Some(List("string", "numeric", "categorical", "timestamp", "boolean"))
        dataSampleTable.rowNames shouldBe None
      ...
      "DataFrame consists of null values only" in {
        ...
        val emptyDataFrame = executionContext.dataFrameBuilder.buildDataFrame(
          schema,
          sparkContext.parallelize(Seq(Row(null, null, null, null, null), Row(null, null, null, null, null), Row(null, null, null, null, null)))
        )
    ...
    "shorten long string values in sample data and in distribution tables" in {
      ...
      val buckets = report.content
        .distributions("string")
        .asInstanceOf[DiscreteDistribution]
        .categories
  ...
  def dataWithColumnsCountOverThreshold() = {
    val fieldNames = for {
    ...
    executionContext.dataFrameBuilder.buildDataFrame(schema, sparkContext.parallelize(Seq(row)))
  }
  ...
  private def testReportTables(
    ...
    val report = dataFrame.report()
    testDataSampleTable(cellValue, columnNameBase, dataFrameColumnsNumber, dataFrameRowsNumber, report)
  ...
  private def testDataSampleTable(
    ...
    val dataSampleTable = report.content.tableByName(DataFrameReportGenerator.DataSampleTableName).get
    val expectedRowsNumber = Math.min(DataFrameReportGenerator.MaxRowsNumberInReport, dataFrameRowsNumber)
    dataSampleTable.columnNames shouldBe Some((0 until dataFrameColumnsNumber).map(columnNameBase + _))
    dataSampleTable.rowNames shouldBe None
    dataSampleTable.values shouldBe List.fill(expectedRowsNumber)(List.fill(dataFrameColumnsNumber)(cellValue))
  }

  private def testDataFrameSizeTable(report: Report, numberOfColumns: Int, numberOfRows: Long) = {
    val dataFrameSizeTable = report.content.tableByName(DataFrameReportGenerator.DataFrameSizeTableName).get
