...
/** Used to hold the data that is being dragged during a drag and drop action. It may hold one or more data items, each of one or more data types. For more information about drag and drop, see HTML Drag and Drop API. */
@js.native
trait DataTransfer extends js.Object {
  /**
    * Returns the kind of action that is currently selected. If the kind of action isn't one of those that is allowed by the effectAllowed attribute, then the action will fail.
    * 
    * Can be set, to change the selected action.
    * 
