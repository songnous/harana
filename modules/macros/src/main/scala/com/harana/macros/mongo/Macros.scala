...
class DynamicCodecRegistry extends CodecRegistry {
  ...
  def get[T](it: Class[T], registry: CodecRegistry): Codec[T] = Try {
    providedCodecs.get(it)
  }.toOption.orElse {
    Some(registered(it).asInstanceOf[Codec[T]])
  }.get
  ...
  val providedCodecs: CodecRegistry =
    CodecRegistries.fromRegistries(
      CodecRegistries.fromCodecs(new DoubleCodec, new IntCodec, new LongCodec, new InstantCodec, new BooleanCodec, new UUIDCodec, new ObjectIdCodec)
    )
...
object CodecGen {
  ...
  def forSealedImpl[T: c.WeakTypeTag](c: whitebox.Context)(registry: c.Expr[DynamicCodecRegistry]): c.Expr[Codec[T]] = {
    ...
    val objCodecs = objects.map(o =>
      q"""${nameOf(o)} -> {
         new com.harana.macros.mongo.ExistentialCodec[$tpe]($o).asInstanceOf[Codec[$tpe]]
        }"""
    )

    val ccCodecs = caseClasses.map(cc =>
      q"${nameOf(cc)} -> com.harana.macros.mongo.CodecGen.gen[$cc]($registry).asInstanceOf[Codec[$tpe]]"
    )
    ...
    val e = c.Expr[Codec[T]] {
      q"""{
        val codec = new org.bson.codecs.Codec[$tpe] {
          ...
          val codecs: Map[String, Codec[$tpe]] = List(..$objCodecs,..$ccCodecs).toMap
        ...
        q"""
          ...
          override def encode(writer: BsonWriter, value: $tpe, encoderContext: EncoderContext) = {
            ...
            if (!codec.isInstanceOf[com.harana.macros.mongo.ExistentialCodec[$tpe]]) {
  ...
  def materializeCodec[T: c.WeakTypeTag](c: whitebox.Context)(registry: c.Expr[DynamicCodecRegistry]): c.Expr[Codec[T]] = {
    ...
    case class EnumerationField(tpe: Type) extends FieldType {
      def codecExpr: Tree = q"""new com.harana.macros.mongo.EnumerationCodec[$tpe]().asInstanceOf[Codec[Any]]"""
    }
    case class SeqField(inner: FieldType) extends FieldType {
      ...
      def codecExpr: Tree = q"""new com.harana.macros.mongo.SeqCodec(${inner.codecExpr}).asInstanceOf[Codec[Any]]"""
    }
    case class SetField(inner: FieldType) extends FieldType {
      ...
      def codecExpr: Tree = q"""new com.harana.macros.mongo.SetCodec(${inner.codecExpr}).asInstanceOf[Codec[Any]]"""
    }
    case class ListField(inner: FieldType) extends FieldType {
      def tpe: c.universe.Type = appliedType(typeOf[List[Any]].typeConstructor, List(inner.tpe))

      def codecExpr: Tree = q"""new com.harana.macros.mongo.ListCodec(${inner.codecExpr}).asInstanceOf[Codec[Any]]"""
    }
    case class MapField(key: FieldType, value: FieldType) extends FieldType {
      ...
      def codecExpr: Tree = q"""new com.harana.macros.mongo.MapCodec[${key.tpe},${value.tpe}](${value.codecExpr}).asInstanceOf[Codec[Any]]"""
    }
    case class EitherField(innerA: FieldType, innerB: FieldType) extends FieldType {
      ...
      def codecExpr: Tree = q"""new com.harana.macros.mongo.EitherCodec(${innerA.codecExpr}, ${innerB.codecExpr}).asInstanceOf[Codec[Any]]"""
    }
      ...
      def decode(reader: BsonReader, decoderContext: DecoderContext):$tpe = ${
        q"{..${
          (q"""reader.readStartDocument""" +:
            ...
            q"""while(reader.readBsonType != BsonType.END_OF_DOCUMENT) ${
              Match(
                ...
                fields.zipWithIndex.map { case (field, ix) =>
                  cq"""${Literal(Constant(field.name.decodedName.toString))} =>
                      ${field.name.toTermName} = Some(_codecs($ix).decode(reader, decoderContext).asInstanceOf[${field.raw.tpe}]) """
                } :+
                  cq"""foo => println("Ignore unmapped field `" + foo + "'"); reader.skipValue()""" //FIXME add tests
              )
            }""" :+
            q"""reader.readEndDocument""") ++
            ...
            q"""new $tpe(${fields.map(f => if (f.option) q"""${f.name.toTermName}""" else q"""${f.name.toTermName}.get"""): _*})"""
        }}"
      }
      }
      codec
