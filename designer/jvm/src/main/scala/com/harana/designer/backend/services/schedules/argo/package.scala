...
//    UIO(schedule.pipelines.map(_.map(p => (p, p.start.triggers.getOrElse(List())))).getOrElse(List()))


//  def argoEventSources(eventBusName: String, schedule: Schedule): UIO[List[EventSource]] =
//    for {
