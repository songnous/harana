...
class StageTimeSpan(val stageID: Int, numberOfTasks: Long) extends TimeSpan {
  var stageMetrics  = new common.AggregateMetrics()
...
object StageTimeSpan {
  ...
  def getTimeSpan(json: Map[String, JValue]): mutable.HashMap[Int, StageTimeSpan] = {
    ...
    json.keys.map(key => {
      ...
      timeSpan.stageMetrics = common.AggregateMetrics.getAggregateMetrics((value \ "stageMetrics")
              .extract[JValue])
      ...
      timeSpan.parentStageIDs = common.Json4sWrapper.parse((value \ "parentStageIDs").extract[String]).extract[List[Int]]
      timeSpan.taskExecutionTimes = common.Json4sWrapper.parse((value \ "taskExecutionTimes").extract[String])
        .extract[List[Int]].toArray

      timeSpan.taskPeakMemoryUsage = common.Json4sWrapper.parse((value \ "taskPeakMemoryUsage").extract[String])
