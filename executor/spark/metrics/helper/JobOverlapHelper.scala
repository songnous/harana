package com.harana.executor.spark.metrics.helper

import com.harana.executor.spark.metrics.common
import com.harana.executor.spark.metrics.timespan.JobTimeSpan
...
object JobOverlapHelper {
  ...
  /**
    ...
    * @return
    */
  def makeJobLists(ac: common.AppContext): List[List[JobTimeSpan]] = {
  ...
  def estimatedTimeSpentInJobs(ac: common.AppContext): Long = {
    val jobGroupsList = makeJobLists(ac)
    jobGroupsList.map(x => (x.map(x => x.endTime).max - x.map(x => x.startTime).min)).sum
  }

  def criticalPathForAllJobs(ac: common.AppContext):Long = {
