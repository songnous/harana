...
class JobOverlapAnalyzer extends AppAnalyzer {
  ...
  def analyze(appContext: common.AppContext, startTime: Long, endTime: Long): String = {
    ...
    val jobsList = helper.JobOverlapHelper.makeJobLists(ac)
    var count = 1
  ...
  def printJobGroupTimeLine(out: mutable.StringBuilder, listJobTimeSpan: List[timespan.JobTimeSpan]): Unit = {
