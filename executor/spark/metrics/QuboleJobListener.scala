...
import com.harana.executor.spark.metrics.timespan.HostTimeSpan
...
class QuboleJobListener(sparkConf: SparkConf)  extends SparkListener {
  ...
  protected val appInfo          = new common.ApplicationInfo()
  protected val executorMap      = new mutable.HashMap[String, timespan.ExecutorTimeSpan]()
  protected val hostMap          = new mutable.HashMap[String, HostTimeSpan]()
  protected val jobMap           = new mutable.HashMap[Long, timespan.JobTimeSpan]
  protected val jobSQLExecIDMap  = new mutable.HashMap[Long, Long]
  protected val stageMap         = new mutable.HashMap[Int, timespan.StageTimeSpan]
  protected val stageIDToJobID   = new mutable.HashMap[Int, Long]
  protected val failedStages     = new ListBuffer[String]
  protected val appMetrics       = new common.AggregateMetrics()
  ...
  private def appAggregateMetrics(): common.AggregateMetrics = appMetrics

  private def hostAggregateMetrics(hostID: String): Option[common.AggregateMetrics] = {
    hostMap.get(hostID).map(x => x.hostMetrics)
  }

  private def executorAggregateMetrics(executorID: String): Option[common.AggregateMetrics] = {
  ...
  override def onApplicationEnd(applicationEnd: SparkListenerApplicationEnd): Unit = {
    ...
    val appContext = new common.AppContext(appInfo,
      appMetrics,
      0, // FIXME
      0, // FIXME
      hostMap,
    ...
    asyncReportingEnabled(sparkConf) match {
      ...
      case false => {
        analyzer.AppAnalyzer.startAnalyzers(appContext)
      }
    }
  }
  ...
  override def onExecutorAdded(executorAdded: SparkListenerExecutorAdded): Unit = {
    ...
    if (executorTimeSpan.isEmpty) {
      val timeSpan = new timespan.ExecutorTimeSpan(executorAdded.executorId,
  ...
  override def onJobStart(jobStart: SparkListenerJobStart): Unit = {
    val jobTimeSpan = new timespan.JobTimeSpan(jobStart.jobId)
