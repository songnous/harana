...
import com.harana.sdk.backend.models.designer.flow.actiontypes.input.next.GetKineticaInfo
...
class GetKinetica extends GetKineticaInfo with ActionType {
  ...
  def execute(inputs: Inputs, parameters: ParameterValues, context: FlowContext): IO[ExecutionError, Option[Outputs]] = {
    ...
    val schema = parameters(schemaParameter)
    val table = parameters(tableParameter)
    val initialOffset = parameters(initialOffsetParameter)
    val maximumResults = parameters(maximumResultsParameter)
    val partitions = parameters(partitionsParameter)
    val appendNewColumns = parameters(appendNewColumnsParameter)
    val mapColumnsByName = parameters(mapColumnsByNameParameter)
